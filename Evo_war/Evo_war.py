...
while status_game:
        ...
        if game_state == 'menu':
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if event.button == 1:
                    ...
                    if selected_unit:
                        x, y = event.pos
                        unit_index = units_on_map.index(selected_unit)
                        army_tab.move_unit(unit_index, x, y)
    ...
    if game_state == 'menu':
        ...
        # Отрисовка юнитов
        for unit, unit_obj in zip(units_on_map, army_tab.units):
            unit_obj.update()
            unit["позиция"] = (unit_obj.x, unit_obj.y)
            unit_obj.draw(screen)
